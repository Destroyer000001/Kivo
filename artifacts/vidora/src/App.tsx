...
import { Bookmark, Check, ChevronLeft, ChevronRight, Clapperboard, Compass, FileVideo, Heart, Home, Menu, MoreHorizontal, Play, Plus, Search, Send, Settings, Share2, Sparkles, Upload, UserRound, Volume2, VolumeX, X } from 'lucide-react';
...
function UploadPage() {
  ...
  if (submitted) return <Shell><div className="mx-auto flex max-w-2xl flex-col items-center py-16 text-center md:py-24"><span className="flex size-16 items-center justify-center rounded-2xl bg-secondary text-secondary-foreground"><Check className="size-8" /></span><p className="mt-7 text-xs font-bold uppercase tracking-[.18em] text-primary">Ready to share</p><h1 className="mt-2 font-mono text-4xl font-bold tracking-[-.06em]">Your video is staged.</h1><p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">This local demo keeps your upload on this screen. In a real KIVO, it would be ready for the community.</p><div className="mt-8 flex flex-wrap justify-center gap-3"><button type="button" data-testid="button-upload-another" onClick={() => { setSubmitted(false); setTitle(''); setDescription(''); setFileName(''); }} className="min-h-11 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground">Upload another</button><Link href="/" data-testid="link-upload-home" className="flex min-h-11 items-center rounded-xl bg-card px-5 text-sm font-bold">Back to home</Link></div></div></Shell>;
